...
<title>PUBLICATION WEB | Complete Review </title>
...
<body <?php if(isset($_POST['edit']) && isset($_POST['tid'])) {echo 'class="sidebar-icon-only"';} ?>>
...
<div class="main-panel">
  <div class="content-wrapper">
    <div class="row">
		...
		<?php
      if ( isset( $_POST[ 'tid' ] ) && isset( $_POST['edit'] ) ) {
       
		  $rthesisquery = "SELECT * FROM `thesis` JOIN user ON `thesis`.`user_id` = `user`.`user_id` where t_id = '" . $_POST[ 'tid' ] . "' ";
          ...
          <div class="card">
            <div class="card-body" style="margin:4px, 4px;height: 600px;overflow-x: hidden;overflow-y: auto; text-align:justify;"> <a class="btn btn-dark btn-rounded btn-fw" href="reviwerTab.php"> <- Reviewer Tab  </a> <BR>
              ...
              <h3 class="mb-4 mt-4" style="font-family: 'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'; "><?php echo strtoupper($thres[2]).'  | (ID : '.$thres[0].')'; ?></h3>
				...
				<p><h3 class="mb-4 mt-4" style="font-family: 'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'; font-size: 14px; color :cadetblue; ">Reviewer (s) : 
			<?php
		  $TCSQLQ = "SELECT `user`.`name` FROM `role` join user on `role`.`reviwer_user_id` = `user`.`user_id` WHERE `role`.`t_id` = '" . $_POST[ 'tid' ] . "'  ";
			...
			?>
				...
				<?php //<form action="completereview.php" method="post" > 
				if(isset($_POST['markcomp'])) {
					echo '<script type="text/javascript"> alert("Marked Completed.."); </script>';
					
				}
				?>
				<form action="completereview.php" method="post" >
				<p><h3 class="mb-4 mt-4" style="font-family: 'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'; font-size: 14px; ">Question 1 : What Can be The Question !..</h3><hr>
	...
	<div class="col-12 grid-margin" >
          <div   align="center">
				...
				<button class="btn btn-success btn-fw" name="markcomp" type="submit" align = "center">Mark Complete </button> 
					</form>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <?php
              $TCSQL = "SELECT user.name, thesis_comments.comment from thesis_comments JOIN user on thesis_comments.user_id = user.user_id WHERE t_id = '" . $_POST[ 'tid' ] . "'  ";
                ...
                <table class="table table-striped">
                  ...
                  <tbody>
                    ...
                    while ( $res = $sql_item->fetch_row() ) {
                      ?>
                    <tr>
                      <td class="py-1" style="color:black;"><?php echo strtoupper($res[5]).'<br>('.$res[1].')';?></td>
                      <td style="color: blue;"><?php echo substr($res[2], 0, 30) . '...';?></td>
                      <td ><?php echo substr($res[3], 0, 40) . '...' ;?></td>
                      <td><?php echo $res[4].' INR';?></td>
                      
						<td><a href="reviwerTab.php?edit=1&tid= <?php echo $res[1]; ?>" class="btn btn-warning mdi mdi-pen"> Review </a>
                    </tr>
                    <?php
                    }
                    ?>
